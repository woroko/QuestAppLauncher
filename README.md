...
The necessary commands for QuestAppLauncher are ```adb shell am start -n "aaa.QuestAppLauncher.App/.AppInfo"``` and ```adb shell cmd package set-home-activity "aaa.QuestAppLauncher.App/.AppInfo"``` to set it as a home activity, replacing the stock launcher. 

If you need to re-enable the stock launcher for some reason, just run ```adb shell pm enable com.oculus.vrshell```
...
If QuestAppLauncher is the home activity but you are stuck at a black screen, you may need to spam the home button a few times to start QuestAppLauncher.
